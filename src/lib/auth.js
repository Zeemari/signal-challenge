...
async function setSession(session) {
  ...
  if (session?.user) {
    const { data, error } = await supabase.from('profiles').select('id, display_name, role, is_active, institution_name, institution_type').eq('id', session.user.id).maybeSingle()
